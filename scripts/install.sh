...
# When this file lives at scripts/install.sh, the Rust workspace root is one directory up.
if [[ "$(basename "$SCRIPT_DIR")" == "scripts" && -f "$SCRIPT_DIR/../Cargo.toml" ]]; then
...
# Launch modes:
# 1) From a checkout: ./install.sh (repo root) or bash scripts/install.sh
