...
- **Construction du HookRunner :** `crate::hooks::hook_runner_from_config` (`src/hooks/mod.rs`) enregistre les builtins configurés lorsque `[hooks].enabled`, et enregistre **`MemoryConsolidationHook`** dès que `memory.auto_save` est vrai (même si les hooks sont désactivés), pour éviter de dupliquer `tokio::spawn` dans les canaux. Avec `[memory.layered]`, les écritures SessionMemory / AutoMemory sur disque passent par `src/memory/consolidation.rs` (même tâche `tokio::spawn`) ; le créneau « tour en attente » entre spawns : `src/memory/layered_context.rs`.
